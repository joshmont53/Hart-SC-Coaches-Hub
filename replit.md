...
- **Intelligent Drill Detection**: AI-powered feature that automatically detects drills mentioned in training session content using GPT-5 Mini. Detected drills are displayed in a dedicated sidebar (accessed via Play button) with expandable cards showing full drill details and embedded videos. The drills sidebar button uses a border-attached design pattern: positioned flush against the right edge of the session content container via a wrapper with `overflow-visible` and negative positioning (`right-[-1px]`), appearing below the distance breakdown button when present. Buttons shift left when sidebars open to remain accessible. Includes fallback case-insensitive substring matching for reliability.
