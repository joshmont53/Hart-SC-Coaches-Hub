...
- **Handbook**: Document management system for coaches to upload, categorize, preview, and download documents (PDF, Word, Excel). Documents are stored in localStorage as base64-encoded data. Features include drag-and-drop upload, 6 document categories (Session Plans, Competition Calendars, Training Programs, Meet Results, Meeting Notes, Other), search and filter functionality, PDF preview with iframe, delete confirmation dialogs, and download capability. Navigation is available in both desktop and mobile sidebars under the SESSIONS section.
