...
- **Session Writer Helper**: A slide-out sidebar panel accessible from the session editor that provides data-driven insights for planning sessions. Features two content layers: (1) Non-AI content showing squad context, recent feedback averages with trends, and highest/lowest category indicators; (2) AI-powered recommendations generated by GPT-5 Mini including "What's Working", "Areas to Address", and focus-specific tips. AI insights are cached per squad/focus combination and persist while the session is being edited, with manual refresh option. Requires minimum 3 sessions with feedback to generate AI insights.
