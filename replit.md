...
- **Multi-Squad Sessions (Phase 2 Complete)**: The `session_squads` junction table enables many-to-many relationships between sessions and squads. The existing `swimming_sessions.squadId` column is preserved as the primary squad for backward compatibility. The `session_squads` table has a unique constraint on (session_id, squad_id) and uses `record_status` for soft deletes (audit trail). Phase 1 (database table) and Phase 2 (session creation UI + attendance) are complete. Both session creation forms (`add-session.tsx` and `new-session.tsx`) now use a multi-select dropdown with checkboxes and badges. Session detail views display all associated squad names and pull swimmers from all squads for attendance. Remaining phases: 3-Calendar display, 4-Edit session, 5-Analytics, 6-Invoice.
