...
### Database
The system utilizes a PostgreSQL database hosted on Neon Serverless, with Drizzle ORM for type-safe interactions. The schema includes core entities such as `users`, `coaches`, `squads`, `swimmers`, `locations`, `swimming_sessions`, `attendance`, `competitions`, `competition_coaching`, and `session_templates`. Relationships are comprehensive, linking various entities. The data model uses UUID primary keys, timestamp tracking, detailed stroke/distance tracking, and template content storage (both plain text and rich HTML). Soft deletes are implemented using a `record_status` column across all core entities.
